...
def generate_grc_layer_network(runID,
    ...
    with open('network_structures/GCLconnectivity_' + str(N_syn) + '.pkl', 'rb') as file:
        p = pkl.load(file)
        conn_mat = p['conn_mat']
    ...
    ls = LEMSSimulation('sim_'+runID, duration, dt) #LEMSSimulation('sim_'+runID, duration, dt, lems_seed = int(np.round(1000*random.random())))
